...
class Event {
  ...
  getEvents = async () => {
    ...
    const res = await get_query(`/events/query/`);
  ...
  getEvent = async (city, date) => {
    ...
    const events_by_date = await get_query(`/events/query/?event_date=${date}`);
  ...
  emailExists = async (email) => {
    try {
      const res = await get_query(`/users/clients/?email=${email}`);
  ...
  phoneExits = async (phone) => {
    try {
      const res = await get_query(
        `/users/clients/?phone_number__icontains=${phone.replace("+", "")}`,
      );
  ...
  getExistingClientForEvent = async (event, client) => {
    try {
      const res = await get_query(
        `/events/shoots/?event=${event}&client=${client}`,
      );
